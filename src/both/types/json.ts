 ...
 */
export function accessor_to_property_names(
  prop_name: string
): (string | number)[] {
  //
  ...
  const prop_parts: string | string[] = "" + prop_name; //clone prop_name
  //
  // replace brackets
  {
    // [ -> .
  ...
  // split by dot
  const prop_parts_withIndex: (string | number)[] = prop_parts.split(".");
 ...
 * @return {string[]}
 */
export function get_accessor_parts(
  accessor: string | (string | number)[]
): (string | number)[] {
  ...
  /*
      ...
      */
  const matches: (string | number)[] = accessor.match(/(?<=\b)[^\.]+(?=\b)/g);

  /**
   * Parse numbers accessors
   */
  if (matches) {
    matches.forEach((accessor: string | number, idx: number) => {
      //
      // Accessor is an integer
      if (/^\d+$/.test(<string>accessor)) {
        matches[idx] = Number.parseInt(<string>accessor);
      }
    });
  }
...
export function prefix_parent_property_names(
  obj,
  charac = "-",
  id_prefix = ""
) {
...
export function merge(
  obj_receiving,
  obj_to_merge,
  keep_obj_receiving_values = true
) {
