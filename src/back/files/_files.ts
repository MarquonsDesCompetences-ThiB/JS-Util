 ...
 * @return {string}
 */
export function cell_ids_to_alphanum(col_id: number, row_id: number): string {
 ...
 * @return {string}
 */
export function column_id_to_letters(
  col_id: number,
  exp: number = 0
): string | any {
  let id = col_id,
...
export function is_directory(path: string): boolean {
 ...
 * @param path
 */
export function read_dir_files_json(path: string): {} {
  let contents = {};
