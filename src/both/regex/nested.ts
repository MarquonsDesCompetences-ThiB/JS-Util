...
export function nested_context_scope(
  chars_idx: number[],
  in_string: string,
  all?: boolean
): number[] {
  ...
  // === BRACES
  {
    ...
    if (starting_scope_idxs) {
      ...
      const scope_idxs = to_1_depth_scopes_idxs(
        starting_scope_idxs,
        ending_scope_idxs
      );
  ...
  function to_1_depth_scopes_idxs(
    starting_scope_idxs: number[],
    ending_scope_idxs: number[]
  ) {
    const scopes_idxs = [];
    for (let i = 0; i < starting_scope_idxs.length; i++) {
      ...
      // Fetch the associate end tag
      {
        ...
        let next_scope = i + 1;
        while (
          next_scope <= starting_scope_idxs.length &&
  ...
  function check_nesteds(scopes_idx: number[]) {
...
export function nested_text_scope(
  chars_idx: number[],
  in_string: string,
  all?: boolean
): number[] {
