...
class File_statics {
   ...
   * @return {string}
   */
  static cell_ids_to_alphanum(col_id, row_id) {
    return File_statics.column_id_to_letter(col_id) + row_id;
  }
   ...
   * @return {string}
   */
  static column_id_to_letters(col_id, exp = 0) {
    let id = col_id,
      letters = "";
    ...
    // Recursive call if next exponential is lower than id
    {
      ...
      if (Math.pow(26, exp_incr) < id) {
        const res = File_statics.column_id_to_letter(col_id, exp_incr);
        id = res.id;
        letters = res.letters;
      }
    }
...
util.obj.Obj.export(module, File_statics);
