 ...
 */
export function select(directory_tree, entries_matching_path, file_in_each_dir_matching_pattern, parent_dir_path) {
    if (!parent_dir_path) {
        ...
        // Fetch directories that match fetch
        {
            ...
            directory_tree.dirs.forEach((subdir_tree, name) => {
                if (fetch.bAll_wilcard || fetch.requested_match.test(name)) {
                    const subtree = select(subdir_tree, fetch.bAll_wilcard && fetch.requested_match.test(name)
