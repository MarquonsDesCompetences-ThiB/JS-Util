...
export class Xlsx_File extends Xlsx_File_props {
  ...
  async read(
    ...
    sheet_id_or_name: string | number = 1
   ...
   */
  read_row(row_id: number, sheet_id_or_name: number | string = 1) {
    {
      if (!this.excel) {
   ...
   *                                    xlsx : Nb cells
   */
  async apply_updates(): Promise<iUpdates_Res> {
    //
    // Excel format
    {
      if (this.excel) {
