  ...
  async read_key(key: string | string[]) {
    return new Promise<string[]>(async (success, reject) => {
      ...
      function new_chars_delimiters_regex() {
        ...
        return new RegExp(
          `${brace_open}|${brace_close}|${bracket_open}|${bracket_close}|${parenthese_open}|${parenthese_close}|${quote}|${double_quotes}|${template_quote}|${comma}`
        );
      }
      ...
      stream.on("data", (chunk) => {
        ...
        // Look for key
          ...
          if (parsing_str[0] !== '"') {
            throw SyntaxError(
              "Expected a key but it has no double-quotes (in : " + parsing_str
            );
          }
          ...
          // Consume the ending double_quotes and semi-column
          {
            ...
            if (!col_idx) {
              throw SyntaxError("No semi-column found at key " + <string>chunk);
            }
