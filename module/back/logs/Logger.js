...
export class Logger {
        ...
        }
        {
            ...
            this.logs.output = (msg) => {
                ...
                if (that.file_desc != null) {
                    ...
                    fs_extra.write(that.file_desc, buffer, 0, buffer.length, null, function (err, writtenbytes) {
                        if (err) {
     ...
     * @return{string}
     */
    static add_debug_infos(message, include_file_infos = true, include_stack_trace = false, row = 0) {
     ...
     */
    set error(message) {
        const formatted = Logger.add_debug_infos(message, true, //file infos
        true // stack trace
        );
    ...
    set ex(ex) {
        ...
        function format_error(error) {
            const msg = ex instanceof Error
                ? ex.name + " : " + ex.message + "\n" + ex.stack
                : ex;
            return Logger.add_debug_infos(msg, true, false, 1 //remove format_error call
            );
        }
    }
    set fatal(message) {
